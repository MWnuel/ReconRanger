    ...
    for host in nm.all_hosts():
        ...
        if 'udp' in nm[host]:   
            for port in nm[host]['udp']:
                cur.execute("SELECT id FROM ports WHERE ip=%s AND port_number=%s AND port_type=%s", (ip, int(port), 'udp', ))
                port_exists = cur.fetchone() 
                if nm[host]['udp'][port]['state'] == 'open':    
                    if not port_exists:
                        ...
                        id = int(port_exists[0])
                        query =  "UPDATE ports SET (port_state, service, version, extrainfo) = (%s,%s,%s,%s) where id = %s "
                        values = ( str(nm[host]['udp'][port]['state']), str(nm[host]['udp'][port]['name']), str(nm[host]['udp'][port]['product'] + ' ' + nm[host]['udp'][port]['version']), str(nm[host]['udp'][port]['extrainfo']), id,)
                        cur.execute(query, values)                   
                    conn.commit()
                    continue
                if not port_exists:          
                    ...
                    id = int(port_exists[0]) 
                    query =  "UPDATE ports SET (port_state, service, version, extrainfo) = (%s,%s,%s,%s) where id = %s "
                    values = ( str(nm[host]['udp'][port]['state']), '', '', '', id,)
                    cur.execute(query, values)
                conn.commit()
